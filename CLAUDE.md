...
- `Duffel.Client` (`lib/duffel/client.ex`) — Req-based transport. It owns:
  - Auth and headers: bearer auth, the `Duffel-Version` header, gzip.
  - `receive_timeout` (130s default) — Duffel allows order/booking creation to take up to 120s and recommends a client timeout slightly above that; searches are quicker, so a search-only client can lower it.
  - Retries (`retry: &retry?/2`) — 408/429/503 and a whitelist of network errors (transport `:timeout`/`:econnrefused`/`:closed`, HTTP/2 `:unprocessed`/`:pool_not_available`) on any method, 504 on `GET`/`HEAD` only. Other transport errors such as `:ehostunreach` are not retried, and never 500/502, which the docs call non-retryable. A retried `POST` can still book twice, because a timeout can land after Duffel has accepted the booking, so a create ending in a transport error or a 5xx has an unknown outcome.
  - The `data` request/response envelope.
  - The `Idempotency-Key` header — auto-generated by default on `post`, overridable or disabled via the `:idempotency_key` opt. Duffel's published docs never mention idempotency keys, so the header is a precaution — do not write docs promising it de-duplicates.
  - Pagination: `list/3` → `Duffel.Page`, `stream/3` → lazy `Stream.resource` following `meta.after` cursors.
