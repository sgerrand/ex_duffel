...
- Resource modules (`lib/duffel/*.ex`, plus `lib/duffel/{stays,cars,identity}/*.ex`) — thin, no macros. Each wraps `Client.get/post/put/patch/delete/list/stream`, piping the result through `Client.unwrap/1` to take the resource out of the `"data"` envelope (a 2xx body without that key becomes an `:unexpected_response` error, never a silent pass-through). `lib/duffel/offer_requests.ex` is the canonical template for new resources.
